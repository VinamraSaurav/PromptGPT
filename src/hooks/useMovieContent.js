    ...
    const fetchTrailerVideo=async()=>{
        try {
            const response = await fetch(
                'https://api.themoviedb.org/3/'+movieId[0]+'/'+movieId[1]+'/videos?language=en-US',
              ImbdOptions
            );
            if (response.ok) {
              ...
              if(movieId[2]===undefined){
                const mainConetent =data?.results[0];
                dispatch(addMainContent(mainConetent));
              }else{
                  const filterData=data?.results.filter(video=>video.id===movieId[2]);
